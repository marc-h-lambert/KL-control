...
The controller is minimizing the finite-time continuous loss: $\int_0^T \ell(x_t,p_t,u_t,t)dt +V_T(X_T)$ which writes in discrete-time state-space represntation with a step $\mathrm{dt}$ where $N\mathrm{dt}=T$:
