...
where $v_t \in \mathcal{M}(d \times 1)$, $u_t \in \mathcal{M}(p \times 1)$, $W_t \in \mathcal{M}(m \times 1)$, $L \in \mathcal{M}(d \times m)$ and $C \in \mathcal{M}(m \times m)$ which is discretized using a semin-implicit scheme as follows:
