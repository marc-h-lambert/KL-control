...
The controller is minimizing the finite-time continuous loss: $\int_0^T \ell(x_t,p_t,u_t,t)dt +V_T(x_T,p_T)$ which writes in the discrete-time-state-space representation with a step $\mathrm{dt}$ and with $N:=T/\mathrm{dt}$:
